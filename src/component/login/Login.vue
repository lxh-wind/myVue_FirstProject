...
export default {
    ...
    return {
      ...
      rules: {
        ...
        uname: [
          ...
          {
            pattern: /\w{4,10}/,
            message: "长度在 4 到 10 个字符, 且只能为字母或数字",
            trigger: "blur"
          }
        ],
        upwd: [
  ...
  methods: {
    ...
    //登录的请求函数
    login() {
      this.$http.post(this.$api.login, this.ruleForm).then(res => {
        // console.log(res);
        if (res.data.status == 0) {
            var nextLogin = this.$route.query.nextLogin;
            console.log(this.$route);
          //$router上有个方法  push  可以直接实现路由的跳转
          this.$router.push(nextLogin ? nextLogin:'admin');
        } else {
